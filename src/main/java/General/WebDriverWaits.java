...
public class WebDriverWaits {
    ...
    public static void visibilityOfMainBoard(){
         wait.until(ExpectedConditions.visibilityOf(driver.findElement(By.id("mainboard"))));
    }
    public static void visibilityOfSchematicView(){
         wait.until(ExpectedConditions.visibilityOf(driver.findElement(By.id("schematicView"))));
    }
    ...
    public static void waitForPageReadyByJquery() {
        int counter = 0;
        if(!(Boolean) ((JavascriptExecutor) driver).executeScript("return (window.jQuery != null) && (jQuery.active === 0);"))
        {
            try {
                Thread.sleep(1000);
    ...
    public static void waitUntilElementVisible(){
        new WebDriverWait(driver, 50).until(ExpectedConditions.invisibilityOfElementLocated(By.className("vj-animated-svg-icon"))); // wait for Loading panel to close
    }

    public static void waitUntilLoaderDisapears(){
        WebElement loader = driver.findElement(By.className("spinner-wrapper"));
...
//        wait.until(ExpectedConditions.attributeContains(role, "class", "SettingsButton-ListItem"));
    ...
    public static boolean waitForElToBeAppeared(String element) {
        try {
            ...
            WebDriverWait wait = new WebDriverWait(driver, 1000, 7000);
